...
# --- 3. MAIN (DEBUG VERSION) ---

if __name__ == "__main__":
    try:
        print("DEBUG: Testing Google Auth...")
        service = get_drive_service()
        print("DEBUG: Auth Successful.")

        print(f"DEBUG: Downloading Config (ID: {CONFIG_FILE_ID})...")
        ...
        print(f"DEBUG: Config loaded for {len(trim_data)} items.")

        print(f"DEBUG: Listing folder (ID: {INPUT_FOLDER_ID})...")
        results = service.files().list(q=f"'{INPUT_FOLDER_ID}' in parents and trashed = false", fields="files(id, name)").execute()
        files = results.get('files', [])
        print(f"DEBUG: Found {len(files)} files in Drive.")
